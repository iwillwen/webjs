 ...
 */
session.start = function (req, res, opt) {
    if (req.cookies == undefined) req.cookies = utils.unserializeCookie(req.headers.cookie) || {};
    var maxAge = opt.cookie.maxAge || session.MAX_AGE;
    // 获取session id
    ...
    if (typeof get[session.SESSION_TAG] == 'string' && get[session.SESSION_TAG] != '')
        ...
        res.setCookie(session.SESSION_TAG, session_id, {maxAge: maxAge});
    }
