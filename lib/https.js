	...
	exports.server = https.createServer(function (req, res) {
			...
			res.sendFile = function (fileName) {
				fs.stat(fileName, function (err, stats) {
					...
					res.writeHead(200, {'Content-Type' : this.charset, 'Date': new Date().toUTCString(), 'Accept-Ranges': 'bytes', 'Last-Modified': lastModified, 'Content-Length': size, 'Server': 'Node.js with webjs'});
					fileStream.pipe(res);
				});
			};
			/*
			...
			if (global.web.set('sessionParse'))
				req.sessionStart();
			try {
				var handlers1 = this.listeners('route');
				if (handlers1.length == 0) res.next();
				for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
				var handlers = global.web.listeners('route');
				if (handlers.length == 0) res.next();
				for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
			} catch(ex) {
				switch (req.method.toLowerCase()) {
					case "post":
						try {
							var handlers1 = this.listeners('post');
							if (handlers1.length == 0) res.next();
							for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
							var handlers = global.web.listeners('post');
							if (handlers.length == 0) res.next();
							for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
						} catch(ex) {
					...
					case "put":
						if (exports.server.config('readonly')) {
							...
							try {
								var handlers1 = this.listeners('put');
								if (handlers1.length == 0) res.next();
								for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
								var handlers = global.web.listeners('put');
								if (handlers.length == 0) res.next();
								for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
							} catch(ex) {
								if (!global.web.set('bodyParse')) {
					...
					case "get":
						try {
							var handlers1 = this.listeners('get');
							if (handlers1.length == 0) res.next();
							for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
							var handlers = global.web.listeners('get');
							if (handlers.length == 0) res.next();
							for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
						} catch(ex) {
					...
					case "delete":
						if (exports.server.config('readonly')) {
							...
							try {
								var handlers1 = this.listeners('delete');
								if (handlers1.length == 0) res.next();
								for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
								var handlers = global.web.listeners('delete');
								if (handlers.length == 0) res.next();
								for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
							} catch(ex) {
					...
					case "head":
						if (exports.server.config('readonly')) {
							...
							try {
								var handlers1 = this.listeners('head');
								if (handlers1.length == 0) res.next();
								for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
								var handlers = global.web.listeners('head');
								if (handlers.length == 0) res.next();
								for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
							} catch(ex) {
					...
					case "trace":
						if (exports.server.config('readonly')) {
							...
							try {
								var handlers1 = this.listeners('trace');
								if (handlers1.length == 0) res.next();
								for (var i = 0;i < handlers1.length; i++) handlers1[i](req, res, function() {return res.continue();});
								var handlers = global.web.listeners('trace');
								if (handlers.length == 0) res.next();
								for (var i = 0;i < handlers.length; i++) handlers[i](req, res, function() {return res.continue();});
							} catch(ex) {
								res.send(req.header);
