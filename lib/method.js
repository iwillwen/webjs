    ...
    web.httpMethodHelper = function (method, handlers, server) {
        ...
        switch (typeof handlers) {
            case 'object':
                var routers = [];
                for (key in handlers) {
                    routerRule[key] = new RegExp('^' + key.replace(/:([a-zA-Z0-9-_$]*)/g, '(.*)').replace(/\(\(\.\*\)\)/, '(.*)') + '$', 'i');
                    _server.on('request', function (_req, _res) {
                        var req = new Request(_req);
                        if (req.method.toLowerCase() === method && routerRule[key].test(req.reqPath)) {
                            ...
                            middlewavesRunner.assign(function () {
                                req.param = utils.restParser(handlers, routerRule[key], req.reqPath);
            ...
            case 'string':
                ...
                _server.on('request', function (_req, _res) {
                    var req = new Request(_req);
                    if (req.method.toLowerCase() === method && routerRule[handlers].test(req.reqPath)) {
                        var res = new Response(_res);
                        res.reqHeaders = req.headers;
                        var middlewavesRunner = new asynclist(middlewaves);
                        middlewavesRunner.assign(function () {
