        ...
        switch (typeof handlers) {
            case 'object':
                for(var key in handlers) {
                    var routerRule = new RegExp('^' + key.replace(/:([a-zA-Z0-9-_$]*)/g, '(.*)').replace(/\(\(\.\*\)\)/, '(.*)') + '$', 'i');
                    var handle = function (_req, _res) {
                        req = new Request(_req);
                        res = new Response(_res);
                        res.reqHeaders = req.headers;
                        if (req.method.toLowerCase() === method && routerRule.test(req.reqPath))
                    ...
                    _server.on('request', handle);
                }
                break;
            case 'string':
                var routerRule = new RegExp('^' + handlers.replace(/:([a-zA-Z0-9-_$]*)/g, '(.*)').replace(/\(\(\.\*\)\)/, '(.*)') + '$', 'i');
                var handle = function (_req, _res) {
                    req = new Request(_req);
                    res = new Response(_res);
                    res.reqHeaders = req.headers;
