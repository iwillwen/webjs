...
function method (web) {
    ...
    web.__defineGetter__('web.__defineGetter__end', function () {
        return function (file) {
            if (arguments.length = 1)
                switch (typeof file) {
                    case "string":
                        var web.__defineGetter__end = require(file);
                        web.__defineGetter__end(this);
                        break;
                    case "object":
                        var web.__defineGetter__ = file.web.__defineGetter__ || file.web.__defineGetter__end;
                        web.__defineGetter__(this);
                        break;
                    case "function":
                        file(this);
                        break;
                ...
                for (var i = 0; i < arguments.length; i++) {
                    switch (typeof arguments[i]) {
                        case "string":
                            var web.__defineGetter__end = require(arguments[i]);
                            web.__defineGetter__end(this);
                            break;
                        case "object":
                            var web.__defineGetter__ = arguments[i].web.__defineGetter__ || arguments[i].web.__defineGetter__end;
                            web.__defineGetter__(this);
                            break;
                        case "function":
                            arguments[i](this);
    ...
    web.__defineGetter__('render', function () {
        return function () {
            ...
            var web.__defineGetter__ = self.set('view engine') || 'jade';
            var engine = require(web.__defineGetter__);
            ...
            if (opt.layout !== false) {
                if (opt.layout === undefined || opt.layout === true) {
                    fs.readFile(root + 'layout.' + web.__defineGetter__, function (err, layout) {
                        if (err) return render.trigger('layout', '#{body}');
                        render.trigger('layout', layout.toString());
                    });
                } else {
                    fs.readFile(root + opt.layout + '.' + web.__defineGetter__, function (err, layout) {
                        if (err) return render.trigger('layout', '#{body}');
                        render.trigger('layout', layout.toString());
                    });
                }
            ...
            view.partial = function (tmlp) {
                return fs.readFileSync(root + tmlp + '.' + web.__defineGetter__);
            };
            fs.readFile(root + name + '.' + web.__defineGetter__, function (err, tmlp) {
...
module.exports.web.__defineGetter__ = method;
