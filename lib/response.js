...
Response.prototype.render = function () {
    ...
    if (engine.clearCache) engine.clearCache();
    ext = global.web.set(ext) || ext;
    var root = layout_view.root;
    var render = new eventproxy();
    var Render = engine.renderText || engine.render;
    if (!/\/$/i.test(root)) root += '/';
    if (view.local) {
        layout_view = view;
    ...
    render.assign('layout', 'body', function (layout, body) {
        ...
        var rendeStream = Render(layout, layout_view, function (err, fin) {
            if (err) return self.sendError(500);
            self.format = 'html';
            self.send(fin, 'text/html');
        });
        if (rendeStream) {
            var complete = new dataStream();
            rendeStream.pipe(complete).on('complete', function () {
                self.format = 'html';
                self.send(complete.body(), 'text/html');
            })
        }
    });
    if (opt.layout !== false) {
    ...
    fs.readFile(root + name + '.' + ext, function (err, tmlp) {
        if (err) return self.sendError(500);
        var rendeStream = Render(tmlp.toString(), view, function (err, body) {
            if (err) return self.sendError(500);
        ...
        if (rendeStream) {
            var complete = new dataStream();
            rendeStream.pipe(complete).on('complete', function () {
                render.trigger('body', complete.body());
            })
        }
    });
};
