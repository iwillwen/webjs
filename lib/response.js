...
module.exports = function (res, req) {
    ...
    res.sendFile = function (fileName) {
        ...
        if ('/' in global.web.server.urlHandlers) {
            if (new RegExp(global.web.server.urlHandlers['/'], 'i').test(fileName)) {
                return send(fileName);
            } else if (/\/$/i.test(global.web.server.urlHandlers['/']) && !/^\//i.test(fileName)) {
                return send(global.web.server.urlHandlers['/'] + fileName);
            } else {
                return send(global.web.server.urlHandlers['/'] + '/' + fileName);
            }
        }
